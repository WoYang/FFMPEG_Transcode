 ...
 *	一种方式是：解复用->视频+音频流->解码->YUV/PCM等->音/视频编码->重新生成的音视频流->复用->合成流
 *	一种方式是：使用组件AVFilter,依靠AVFilter做转码效率比第一种方式高,并且解码的CPU和时间消耗也少的多
 */
    ...
    AVCodecContext *enc_ctx, const char *filter_spec)
{
  ...
  //add for single stream mode
  if(dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO && disable_audio){
    goto end;
  }else if(dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO && disable_video){
    goto end;
  }
