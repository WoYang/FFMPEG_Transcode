...
int attribute_align_arg avcodec_send_packet(AVCodecContext *avctx, const AVPacket *avpkt)
{
    ...
    if (!avcodec_is_open(avctx) || !av_codec_is_decoder(avctx->codec)){
		av_log(avctx, AV_LOG_ERROR, "av_codec error.\n");
		return AVERROR(EINVAL);
	}
        
    if (avctx->internal->draining){
		av_log(avctx, AV_LOG_ERROR, "av_codec draining.\n");
		return AVERROR_EOF;
	}
        
    if (avpkt && !avpkt->size && avpkt->data){
		av_log(avctx, AV_LOG_ERROR, "av_codec avpkt error.\n");
		return AVERROR(EINVAL);
	}
     
    ret = bsfs_init(avctx);
    if (ret < 0){
		av_log(avctx, AV_LOG_ERROR, "av_codec bsfs_init error.\n");
		return ret;
	}
    ...
    if (avpkt && (avpkt->data || avpkt->side_data_elems)) {
        ...
        if (ret < 0){
			av_log(avctx, AV_LOG_ERROR, "av_codec av_packet_ref error.\n");
			return ret;
		}
    }
    ...
    if (ret < 0) {
		av_log(avctx, AV_LOG_ERROR, "av_codec av_bsf_send_packet error.\n");
        av_packet_unref(avci->buffer_pkt);
        return ret;
    }
    ...
    if (!avci->buffer_frame->buf[0]) {
        ...
        if (ret < 0 && ret != AVERROR(EAGAIN) && ret != AVERROR_EOF){
			av_log(avctx, AV_LOG_ERROR, "av_codec decode_receive_frame_internal error.\n");
			return ret;
		}  
    }
